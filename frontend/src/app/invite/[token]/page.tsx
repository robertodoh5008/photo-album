...
const ROLE_LABELS: Record<string, string> = {
  viewer: "Viewer — you can browse photos and videos",
  contributor: "Contributor — you can browse and upload media",
};
...
export default function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await publicFetch<InvitePreview>(`/public/invites/${token}`);
        setPreview(data);
      } catch {
        setNotFound(true);
      } finally {
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      setAccepted(true);
      setTimeout(() => {
        if (preview?.album_id) router.push(`/albums/${preview.album_id}`);
      }, 1500);
    } catch (err) {
  ...
  if (notFound) {
    return (
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        <p className="text-gray-500 mb-6">This invite link is invalid, has expired, or was revoked.</p>
  ...
  if (preview?.status === "revoked") {
    return (
      <InviteStatus
        ...
        message="The album owner has revoked this invite link."
      />
    );
  }
  ...
  if (preview?.status === "expired") {
    return (
      <InviteStatus
        ...
        message="This invite link has expired. Ask the album owner to send a new one."
      />
    );
  }
  ...
  if (preview?.status === "accepted" && !accepted) {
    return (
      <InviteStatus
        ...
        linkHref={preview.album_id ? `/albums/${preview.album_id}` : "/gallery"}
  ...
  if (accepted) {
    return (
      <InviteStatus
        emoji="🎉"
        title="You're in!"
        message={`You now have ${preview?.role} access to "${preview?.album_name}". Redirecting…`}
      />
    );
  }
  ...
  return (
    <div className="flex items-center justify-center min-h-[60vh] px-4">
      <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8 w-full max-w-md text-center">
        ...
        <p className="text-sm text-purple-600 font-medium mb-1">You&apos;ve been invited</p>
        <h1 className="text-2xl font-bold text-gray-900 mb-1">{preview?.album_name}</h1>
...
function InviteStatus({
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-20 text-center">
      ...
      {linkHref && linkLabel && (
        <Link href={linkHref} className="text-purple-600 font-medium hover:underline">
          {linkLabel}
        </Link>
      )}
    </div>
